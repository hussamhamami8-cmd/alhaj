/* =========================================================
   دليل الحاج — app.js v12 (نهائي ومستقر)
   - يعتمد على جدول pilgrims الجديد
   - تم إصلاح مشكلة تسجيل الدخول نهائياً
   - يعمل مع Supabase مباشرة
========================================================= */
...
function progressData() {
  const done = journey.filter(item => completed.includes(item.id)).length;
  ...
  if (name === "places") {
    ...
    html += duas.map((item, index) => `
      <div class="info dua-card">
        ...
        <div><span class="pill">دعاء</span><h3>${escapeHTML(item.title)}</h3><p>${escapeHTML(item.text)}</p></div>
      </div>
    `).join("");
  } else if (name === "guide") {
    ...
    html = `
      <div class="info pilgrim-settings-card" onclick="showPage('pilgrim')">
        ...
        <div><span class="pill">بطاقة الحاج</span><h3 class="pilgrim-name">${currentPilgrim ? escapeHTML(currentPilgrim.name) : "تسجيل دخول الحاج"}</h3><p>${currentPilgrim ? "عرض بيانات الحاج والموقع" : "افتح بطاقة الحاج من شاشة الدخول"}</p></div>
      </div>
      ...
      <div class="info"><h3>تقدم الرحلة</h3><p>أنجزت ${p.done} من ${p.total} مراحل (${p.percent}%).</p><button class="danger-btn" type="button" onclick="resetProgress()">إعادة ضبط تقدم الرحلة</button></div>
      <div class="info"><h3>حول التطبيق</h3><p>دليل الحاج رفيق رقمي للمعلومات الأساسية المتعلقة بالمناسك والأماكن والأدعية والإرشادات.</p><p class="muted-note">في المسائل الشرعية التفصيلية يُرجع إلى عالم أو مرشد حج موثوق.</p></div>
    `;
  }
...
function renderPilgrimPage() {
  ...
  return `
    ...
    <div class="info">
      <h3>خصوصية الموقع</h3>
      <p>يعرض التطبيق الموقع المرتبط ببطاقة هذا الحاج فقط.</p>
    </div>
  `;
}
...
function openPlace(index, push = true) {
  ...
  if (!place) return;
  if (push) {
    history.pushState({ type: "place", index }, "", "#place-" + index);
  }
  const main = $("#main");
  const page = $("#page");
  const detail = $("#detail");
  ...
  content.innerHTML = `
    ...
    <div class="info detail-info">
      ...
      <p>${escapeHTML(place.text)}</p>
      <h3>معلومات مهمة</h3>
      <ul>${place.details.map(detail => `<li>${escapeHTML(detail)}</li>`).join("")}</ul>
...
function openJourney(index, push = true) {
  ...
  if (!item) return;
  if (push) {
    history.pushState({ type: "journey", index }, "", "#journey-" + index);
  }
  const main = $("#main");
  const page = $("#page");
  const detail = $("#detail");
...
function handleRoute() {
  ...
  const validPages = ["places", "rituals", "duas", "guide", "favorites", "settings", "pilgrim"];
